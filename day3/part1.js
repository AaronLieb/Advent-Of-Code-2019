...
function toCartesian(input) {
  ...
  for (move of input) {
    ...
    let point = {x: prev.x + d.x, y: prev.y + d.y};
    points.push(point)
  }
  return points;
}

function findIntersections(input, points) {
  let next,prev = {x:0,y:0}
  let minDistance = 0;
  for (move of input) {
    ...
    for (let i=0; i < points.length - 1; i++) { // Compare every line segment and see if they intersect
      ...
      if (seg1.v != seg2.v) {
        ...
        if (inBetween(vert.a.y,horz.a.y,vert.b.y) && inBetween(horz.a.x,vert.a.x,horz.b.x)) { // intersect forumla
          let dis = Math.abs(vert.a.x) + Math.abs(horz.a.y)
          console.log("Intersection: " + dis)
